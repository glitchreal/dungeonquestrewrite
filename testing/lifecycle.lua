...
check('Carry does not inherit Host start-delay counter',function()
    local w,s=fixture('Carry');w.ready=true;w.started=false;w.tick(0);w.tick(147)
    assert(s.status=='Waiting for Host to start' and s.readyAt==nil and w.count('changeStartValue')==0)
end)
